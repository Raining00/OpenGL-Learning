...
namespace Renderer
{
    ...
    public:
        ...
        unsigned int loadTexture2D(const std::string& name, const std::string& path, const TextureType& textureType = TextureType::DIFFUSE, glm::vec4 bColor = glm::vec4(1.0f));
        unsigned int loadTexture2D(const std::string& name, unsigned char* images, int width, int height, int channels, const TextureType& textureType = TextureType::DIFFUSE);
        ...
        unsigned int loadTextureDepthCube(const std::string& name, int width, int height);
